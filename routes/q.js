  ...
  if (req.cookies.user) {
    ...
    sqldb.query(question, (err, fetchUserFromCookie) => {
      ...
      if (Object.keys(fetchUserFromCookie).length != 0) {
        ...
        let getAccountPostRecord =
          `SELECT * FROM profiles WHERE email = ` +
          sqldb.escape(gotUserFromCookie.email)
  ...
  if (req.body.bt != "" || req.cookies.user == undefined) {
    ...
    sqldb.query(getCookieUser, (err, cookieUser) => {
      ...
      sqldb.query(postQuestion, question, (err, result, fields) => {
        ...
        console.log(
          `you asked: ${req.body.q} and you ref is ${qRef} and your question ID is ${question.id}`
        )
...
approuter.get("/data/:id", (req, res) => {
  ...
  let getQuestionData =
    `SELECT * FROM questions WHERE refID =` + sqldb.escape(id)
  ...
  if (req.cookies.user != undefined) {
    ...
    sqldb.query(getLoggedInUser, (err, retLoggedInUser) => {
      ...
      if (Object.keys(retLoggedInUser).length != 0) {
        ...
        let getQuestionData =
          `SELECT * FROM questions WHERE ownerID = '${loggedInUser.id}' AND refID =` +
          sqldb.escape(req.params.id)
  ...
  if (req.cookies.user) {
    ...
    sqldb.query(verifyUser, (err, userToVerify) => {
      ...
      if (Object.keys(userToVerify).length != 0) {
        ...
        console.log(
          `${req.body.username} found with cookie ${req.cookies.user} and can update question`
        )
      } else {
      ...
      let verifyQuCon =
        `SELECT * FROM questions WHERE refID = ` + sqldb.escape(req.body.refID)
      sqldb.query(verifyQuCon, (err, quCon) => {
        if (Object.keys(quCon).length != 0) {
          //check for connection
          if (quCon[0].ownerID == userToVerify[0].id) {
            console.log(
              `${req.body.username} found with cookie ${req.cookies.user} and can update question and ${quCon[0].ownerID} is equal to ${userToVerify[0].id} for sure can update`
            )
            //update question
            ...
            sqldb.query(updateQuestion, (err, doneUpdating) => {
              if (err) throw err
              if (
                doneUpdating.changedRows != 0 &&
                doneUpdating.changedRows == 1
              ) {
