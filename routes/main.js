...
sqldb.connect((err) => {
  ...
  console.log(
    `Connected to Main ${process.env.awsserver} on thread: ${sqldb.threadId}`
  )
})
...
approuter.post("/:username", (req, res) => {
  ...
  if (userBrowserCookie) {
    ...
    let verifyCookie =
      ...
      sqldb.escape(userBrowserCookie) +
      `AND username = ` +
      sqldb.escape(currentUser)
    sqldb.query(verifyCookie, (err, safeCookie) => {
      if (err) throw err
      if (Object.keys(safeCookie).length != 0) {
        sqldb.query(
          "UPDATE profiles SET name = ?, username = ?, dob = ?, hint = ? WHERE cookie = ?",
          [
            `${newUserData.accountname}`,
          ...
          (err, result, fields) => {
            ...
            let getUpdatedUser =
              ...
              sqldb.escape(userBrowserCookie) +
              `AND username = ` +
              sqldb.escape(newUserData.username)
            sqldb.query(getUpdatedUser, (err, gottenNewUser) => {
              ...
              if (Object.keys(gottenNewUser).length != 0) {
                ...
                console.log(
                  "Could not fetch account that was just saved :( retry"
                )
                res.json({
                  newUserData,
